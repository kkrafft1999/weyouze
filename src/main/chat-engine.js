...
const { isAbortError, createChatAbortError, describeFetchError } = require('./providers/stream-helpers');
...
function workspaceSystemPrompt(workspaceRoot, selectedRelPath, selectedIsDirectory, pathMod, toolsPrompt) {
...
function createChatEngine({
  ...
  async function resolveProviderContext() {
    ...
    if (!provider) {
      return {
        error: createChatErrorResult({
          error: `Unbekannter Provider: ${activeId}.`,
    ...
    return { activeId, provider, providerConfig, model, effectiveConfig };
  }
  ...
  async function explain({ payload }) {
    ...
    try {
      ...
      if (providerContext.error) return providerContext.error;
      const { activeId, provider, providerConfig, model, effectiveConfig } = providerContext;
      if (provider.fields?.apiKey && !providerConfig?.apiKey) {
        return createChatErrorResult({
          error: `Kein API-Key für ${provider.name} hinterlegt.`,
      ...
      return createChatErrorResult({
        error: describeFetchError(error, 'dem Provider'),
  ...
  async function send({ sessionId, payload, onEvent }) {
    ...
    try {
      ...
      if (workspaceRoot && typeof payload?.selectedPath === 'string' && payload.selectedPath.trim()) {
        const absolutePath = pathMod.resolve(payload.selectedPath.trim());
        const relativePath = pathMod.relative(workspaceRoot, absolutePath);
        if (!relativePath.startsWith('..') && !pathMod.isAbsolute(relativePath)) {
          selectedRelPath = relativePath || '.';
      ...
      for (let round = 0; round < toolRoundLimit; round += 1) {
        ...
        for (const toolCall of toolCalls) {
          ...
          let output;
          try {
            output = await toolRegistry.execute(toolName, args, { workspaceRoot, abortSignal, allowWrite });
          } catch (error) {
